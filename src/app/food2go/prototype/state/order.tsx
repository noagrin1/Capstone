...
// PROTOTYPE FEATURE: Order placement + tracking simulation + history
...
function newId() {
  return `o_${Math.random().toString(16).slice(2)}_${Date.now().toString(16)}`;
}
    ...
    () => ({
      ...
      updateStatus: (orderId, status) => {
        ...
        setActiveOrder((prev) =>
          prev && prev.id === orderId
            ? { ...prev, status, statusUpdatedAtIso: now }
            : prev,
        );
        setHistory((prev) =>
          prev.map((o) =>
            o.id === orderId ? { ...o, status, statusUpdatedAtIso: now } : o,
          ),
        );
      },
      submitFeedback: (orderId, { rating, comment }) => {
        ...
        setHistory((prev) =>
          prev.map((o) =>
            o.id === orderId
              ? { ...o, feedback: { rating, comment, submittedAtIso: now } }
              : o,
          ),
        );
      },
      resetActive: () => setActiveOrder(null),
    }),
    [activeOrder, history],
  );
