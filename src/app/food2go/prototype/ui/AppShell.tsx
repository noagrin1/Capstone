        ...
        <header className="sticky top-0 z-20 bg-white/90 backdrop-blur border-b border-slate-200">
          <div className="px-4 pt-4 pb-3">
            <div className="flex items-center justify-between">
              ...
              <Link
                href="/food2go/cart"
                className="relative rounded-full border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 hover:bg-slate-50"
              >
                Cart
                {totalItems > 0 ? (
          ...
          {showBanner ? (
            <div className="px-4 pb-3">
              <Link
                href="/food2go/tracking"
                className="block rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-900"
              >
                <span className="font-semibold">Order update:</span>{" "}
                {statusLabel(activeOrder.status)} • Tap to track
              </Link>
            </div>
          ) : null}
        </header>
