    ...
    return {
      ...
      addItem: (r, item) => {
        setRestaurant((prevRestaurant) => {
          const nextRestaurant = { id: r.id, name: r.name, deliveryFee: r.deliveryFee };
          setItems((prevItems) => {
            const switching = prevRestaurant && prevRestaurant.id !== r.id;
            if (switching) {
              return [{ id: item.id, restaurantId: r.id, name: item.name, price: item.price, quantity: 1 }];
            }
            const existing = prevItems.find((p) => p.id === item.id);
            if (!existing) return [...prevItems, { id: item.id, restaurantId: r.id, name: item.name, price: item.price, quantity: 1 }];
      ...
      setQuantity: (menuItemId, quantity) => {
        ...
        setItems((prev) => prev.map((p) => (p.id === menuItemId ? { ...p, quantity: q } : p)));
      },
